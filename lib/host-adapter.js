...
// 文件直读会话历史：JSONL → [{role, text, thinking}] 顺序展开（user/assistant 独立消息，
// 不配对）。背景：直连模式 appendFileSync 写入的行，host 的 session:history 走会话缓存读不到，
// 前端 loadHistory/pollReply 取历史必须文件直读（文件是事实源，host 管道会话的 JSONL
// 也是同一格式）。输出与 routes/api.js 的 normalizeHistory 同构（前端契约 {role,text,thinking}）：
//   - thinking 数组块提取（content 数组里的 {type:'thinking'} 块）
//   - 空消息过滤同口径：assistant 空文本且无思考、user 空文本均过滤
//   - 孤立 user（未配对）保留
//   - limit 取最近 N 条（与 readHistory 的 limit 语义一致）
// 文件不存在/不可读返回 []（调用方据此判断降级）。复用 parseSessionJsonl 的解析口径
// （JSON 行过滤、type==='message'、content 字符串/数组兼容），不破坏现有导出。
export function readSessionHistory(filePath, limit = 200) {
  let raw;
  try {
    raw = fs.readFileSync(filePath, 'utf8');
  } catch {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (typeof m.content === 'string') {
      ...
      const t = m.content.find((c) => c && c.type === 'thinking' && typeof c.thinking === 'string');
      thinking = t ? t.thinking : '';
    }
    // 空消息过滤（与 normalizeHistory 同口径）：assistant 空文本且无思考、user 空文本
    if (m.role === 'assistant' && text.trim() === '' && thinking.trim() === '') continue;
    if (m.role === 'user' && text.trim() === '') continue;
    out.push({ role: m.role, text, thinking });
  }
  // limit：保留最近 N 条（前端 200 语义与 readHistory 一致）
