...
// host 依赖已全部迁出（HOST_ADAPTER.md 迁移步骤 5）：model:sample-text 摘要调用
// 经 loadAdapter().sampleText 走 lib/host-adapter.js，本文件不再直接碰 host API。
...
async function loadAdapter() {
  return _adapter ??= import(`./host-adapter.js?t=${Date.now()}`);
}
...
export async function summarizeOld(ctx, oldRounds) {
  ...
  try {
    ...
    if (typeof ctx.bus?.request === 'function') {
      // 摘要采样走 adapter（HOST_ADAPTER.md 迁移步骤 5）；sampleText 内含 bus 缺失防御
      res = await (await loadAdapter()).sampleText(ctx, payload);
    }
