...
export function buildReferenceContext(rounds, opts = {}) {
  const { windowSize = 30, includeThinking = true, maxTotalChars } = opts;
  if (!rounds.length) return '';
  ...
  // 每轮上限，保证总注入量可控；不传则保持原每轮 4000 上限。
  const perRoundMax = maxTotalChars
    ? Math.max(500, Math.floor(maxTotalChars / Math.max(1, recent.length)))
    : 4000;
  recent.forEach((r, idx) => {
    ...
    lines.push(formatRound(round, { includeThinking, perRoundMax }, rounds.length - recent.length + idx));
  });
  return lines.join('\n');
}
