    ...
    messages: Array.isArray(messages)
      ? messages
          .map((m) => {
            ...
            // 第一版仅文本：content 必须为字符串；数组形态（多模态预留）显式丢弃，
            // 避免发送端透传、接收端丢弃的不对称（红队 2026-08-17 P2-8）
...
export async function streamChat(opts = {}) {
  ...
  // 红队 2026-08-17 P2-7：正常结束但内容全空时标记 empty（调用方不再把空回复当成功），
  // 同时保留 ok:true 语义（协议层成功，业务层判空）
  if (!content && !reasoning) {
    return { ok: true, content: '', reasoning: '', model, usage, empty: true };
  }
